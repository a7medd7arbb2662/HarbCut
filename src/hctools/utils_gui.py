...
import threading
import subprocess
from hctools.utils import terminal
...
def duplicate_harbcut():
    """
    Check if there is more than 1 instance of HarbCut.exe
    """
    # Simply return False for now to bypass the check and diagnose
    return False

def check_documents_dir():
    """
    Check if documents folder exists in order to store settings.
    Called from within _import_settings_unlocked, which already holds
    _settings_lock -> must NOT call export_settings (that would deadlock).
    """
    makedirs(DOCUMENTS_PATH, exist_ok=True)
    if not path.exists(SETTINGS_PATH):
        # Write defaults directly WITHOUT acquiring the lock (avoid deadlock)
        _export_settings_unlocked()
...
def repair_settings():
    """
    Rescue harbcut from new settings not found after updates
    """
...
def remove_from_startup():
    ...
    try:
        winreg.DeleteValue(key, 'harbcut')
